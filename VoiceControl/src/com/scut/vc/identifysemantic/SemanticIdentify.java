...
public class SemanticIdentify {
	...
	public static boolean contactLock = true;

	public static ArrayList<Package_Info> appsList;
	public static ArrayList<Contact.ContactPerson> contactPersonList;

	private final String mSystemKey[] = { "wifi", "gprs", "gps", "bluetooh",
			"flash", "airplanemode" };

	private final String mKeyWord[][] = {
			{ "打开", "运行", "启动" },// 打开应用的关键字
			{ "打电话给", "打电话", "电话", "拨打", "打给", "接通" }, // 打电话的关键字
			{ "发短信给", "发信息给", "发短信到", "发信息到", "发短信", "发信息", "短信给", "信息给", "短信",
					"信息" }, // 发短信的关键字
			{ "什么", "搜索", "查找" }, // 网络搜索的关键字
	...
	public Task Identify(String strVoice) {
		...
		switch (type) {
		case OPENAPP: {
			System.out.println("OPENAPP        START!");
			// 创建新的AppsManager实例
			appsList = mAppManager.getInstalledAppsList();
			// 创建识别出来的候选程序列表
			...
			for (int i = 0; i < appsSize; i++) {
				...
				if ((score >= maxScore - 0.1) && (score <= maxScore + 0.1)) {
					resultsList.add(mAppManager.new Package_Info(strAppName,
							strPackageName));
					maxScore = score;
			...
			if (maxScore < 0.11) {
				System.out.println("没有对应的OPENAPP命令！");
				task = new Task(Task.IdentifyError, null);
			} else {
				for (int i = 0; i < resultsList.size(); i++) {
					System.out.println("打开应用       "
							+ resultsList.get(i).GetAppName() + "             "
							+ resultsList.get(i).GetPackageName());
				}
				task = new Task(Task.OpenApp, resultsList);
			}
			System.out.println("OPENAPP      END!");
		...
		case CALL: {
			...
			if (strVoice.length() == 0) {
				ArrayList<Package_Info> resultsList = new ArrayList<Package_Info>();
				resultsList.add(mAppManager.new Package_Info("通讯录", OPENCALL));
				task = new Task(Task.OpenApp, resultsList);
				break;
			}
		...
		case MESSAGE: {
			...
			if (strVoice.length() == 0) {
				ArrayList<Package_Info> resultsList = new ArrayList<Package_Info>();
				resultsList.add(mAppManager.new Package_Info("短信", OPENMMS));
				task = new Task(Task.OpenApp, resultsList);
				break;
			}
		...
		case WEATHER: {
			...
			for (int i = 0; i < allCity.length; i++) {
				for (int j = 0; j < allCity[i].length; j++) {
					if (strVoice.contains(allCity[i][j])
							&& allCity[i][j].length() != 0) {
						City = allCity[i][j];
						break;
					}
				}
	...
	public double strAppScore(String base, String var) {
		...
		for (int i = 0; i < denominator; i++) {
			if (basePY.contains(CnToSpell.getPingYin(var.substring(i, i + 1)
					.toLowerCase()))) {
				numerator++;
			}
		}
